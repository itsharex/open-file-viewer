...
const matrix: FormatCase[] = [
  ...
  {
    plugin: cadPlugin(),
    extensions: ["dxf", "dwg", "dwf", "step", "stp", "iges", "igs", "ifc", "sat", "sab", "x_t", "x_b", "3dm", "skp", "sldprt", "sldasm", "gds", "gdsii", "oas", "oasis"],
...
function routingCases(): Array<{ extension: string; mimeType: string; expected: string }> {
  return [
    ...
    { extension: "gdsii", mimeType: "application/x-gdsii", expected: "cad" },
...
function mimeOnlyRoutingCases(): Array<{ mimeType: string; expected: string }> {
  return [
    ...
    { mimeType: "application/gdsii", expected: "cad" },
